...
void ADC_CallBack(void){
	...
	if(Value != oldVal){
		...
		uint16_t perc = ((uint32_t)Value*100/1023);
		LCD_gotoXY(0,9);
		sprintf(x,"%u",perc);
		LCD_send_string(x);
		LCD_gotoXY(0,12);
...
int main(void)
{
	GPIO_Port_Config(GPIOD,GPIO_OUTPUT);
	GPIO_Port_Config(GPIOC,GPIO_OUTPUT);
	GPIO_Pin_Config(GPIOA,Px0,GPIO_INPUT);
	
	LCD_lcd_init();
	
	ADC_Config_t ADCcfg;
	ADCcfg.Alignment = Right_Adjust;
	...
	ADCcfg.Volt_ref = ADC_Voltage_Ref_AREF;
	ADC_Init(&ADCcfg);
